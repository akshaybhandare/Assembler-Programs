...
//  clrscr();
  ...
  scanf("%x",&progaddr);
  csaddr=progaddr;
  fprintf(fp2,"CS_ADDR\tDEF\tADDRESS\tLENGTH\n");
  fprintf(fp2,"\n");
  fscanf(fp1,"%s",input);
  while(strcmp(input,"END")!=0)
  {
   if(strcmp(input,"H")==0)
   {
    ...
    strcpy(es[count].extdef,"  ");
    fscanf(fp1,"%x",&add);
    es[count].address=add+csaddr;
    fscanf(fp1,"%x",&len);
    es[count].length=len;
    fprintf(fp2,"%s\t%s\t\t%x\t%x\n",es[count].csname,es[count].extdef,es[count].address,es[count].length);
    count++;
   }
